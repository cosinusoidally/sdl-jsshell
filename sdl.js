...
candidate/ builds live in . and can be verified using the relevant en-US firefox*.checsums and firefox*.checsums.asc files. Later Candidate builds are actually copies of the release builds (if you get the right one).
...
event=new Uint8Array(event_raw); // this is an Uint8Array view onto the raw event data. We can then decode the bytes ourself 
...
At this point I am ready to start implementing my "sdl_init" function and my "sdl_mainloop". To avoid poluting the global namespace I should attach them to an object. The sensible object to attach them to would be the sdl object itself.
