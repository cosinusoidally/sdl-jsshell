...
This was written x86_64 Debian Jessie system. I test both 32 bit x86 and
x86_64. I also test the win32 verion in WINE. This code should work on any
x86/x86_64 Linux system with SDL 1.2 from about CentOS 6 up. It should also
work on any version of Windows from XP up. Docs on setting up for win32 are in
setup-win32.js . This code should work on pretty much any version of jsshell,
but the version I have used is 45.8.0 .
...
event loop. Events include mouse movement, joystic events, and keystrokes.
...
frame. We will use a periodic timer to trigger the drawing of a frame. The
frame timer will be generated by SDL_AddTimer and will be used to periodically
wake up our mainloop to draw a frame (since jsshell does not have setInterval
or setTimout (it does have Date.now, but that means we would need to poll to
measure time, which is inefficient (yes I know nested parentheticals in writing
are bad))).
...
function sdl_init(width,height){
  ...
  // SDL_AddTimer schedules the drawing of frames.
  // _.wait_for_next_frame() (further down) will be woken up then the timer next
  // fires
...
This is (approximately) our mainloop. Note the wait_for_next_frame function.
This allows us to go to sleep and idle until we need to actually do some more
work. This saves wasting CPU time. When we wake up we then need to drain the
event queue and dispatch all events to the relevant event listeners. The while
loop containing SDL_PollEvent is used to drain the event queue. SDL_PollEvent
will return false (or whatever the numerical equivalent is) once the queue has
been drained. process_event will be used to process the events. The user
provided render function will actually paint our frame. If running is set to
false we will stop looping and call SDL_Quit to cleanly shut down SDL.
...
Our render function will paint to the back buffer. We have picked a 32 bit
software mode, so this will give us a linear frame buffer to draw to (scans out
to the screen from top left to bottom right in horizontal rows). surface.pixels
will contain the pixel data, which will be in Blue, Green, Red, Alpha order (I
think, could also be Red, Green, Blue, Alpha, if your colours look weird you
probably are painting the channels in the wrong order). All channels are 8
bits. The Alpha channel is ignored (maybe alpha channel is the wrong term,
"padding" might be more appropriate). 
...
to go to sleep and some how wake up when the timer fires. Somehow the callback
needs to wake up the main loop. The approach I chose was to use SDL mutex and
condition variable facilities.
...
libcb.mut is a mutex that must be locked before calling SDL_CondWait. We do
this is sdl.sdl_init, sdl.SDL_CondWait will temp unlock it, but it gets
relocked when sdl.SDL_CondWait returns.

libcb.cond is a condition variable.
